...
local W = {}
arrow_widget_set = W           -- object name

local I = {}
arrow_icon_set = I             -- object name
...
W.cpu = lain.widgets.cpu({
    settings = function()
        local fg_color   = "#000000" -- "#e33a6e"
        widget:set_markup(markup(fg_color, cpu_now.usage .. "% "))
    end
})
...
W.temp = lain.widgets.temp({
    settings = function()
        local fg_color   = "#000000" -- "#f1af5f"
        widget:set_markup(markup(fg_color, coretemp_now .. "°C "))
    end
})
